...
identifier = 21 if request_type == "disconnect" else 22
packet = bytearray(20)
packet[0] = code
packet[1] = identifier
...
tmp_dir=$(mktemp -d)
iface_name="wg-s3-$RANDOM"
client_ns="wg-s3-ns-$RANDOM"
client_iface="wg-client0"
host_veth="vhs3-$RANDOM"
client_veth="vns3-$RANDOM"
radius_container="wg-radius-fr-phase3-soak-$RANDOM"
radius_image="freeradius/freeradius-server:latest"
radius_auth_port=18200
radius_acct_port=18210
coa_port=37992
radius_secret="integration-secret"
radius_log="$tmp_dir/log/phase3-soak.log"
...
server_wg_ip="10.60.0.1/24"
client_wg_ip="10.60.0.2/32"
daemon_pid=""
cycles=3
...
cat >"$tmp_dir/freeradius/mods-enabled/wg_phase3_soak_auth_request" <<EOF
linelog wg_phase3_soak_auth_request {
    filename = /var/log/freeradius/phase3-soak.log
    permissions = 0644
    format = "auth-request user=%{%{User-Name}:-<none>}"
}
EOF

cat >"$tmp_dir/freeradius/mods-enabled/wg_phase3_soak_access_accept" <<EOF
linelog wg_phase3_soak_access_accept {
    filename = /var/log/freeradius/phase3-soak.log
    permissions = 0644
...
cat >"$tmp_dir/freeradius/mods-enabled/wg_phase3_soak_accounting" <<EOF
linelog wg_phase3_soak_accounting {
    filename = /var/log/freeradius/phase3-soak.log
    permissions = 0644
...
server default {
    ...
    post-auth {
        wg_phase3_soak_access_accept
    }

    accounting {
        wg_phase3_soak_accounting
        ok
    }
}
EOF
...
for cycle in $(seq 1 "$cycles"); do
    ...
    wait_for_count "auth-request user=$accept_public_key" "$radius_log" "$cycle" 120 0.1
    wait_for_count "access-accept user=$accept_public_key" "$radius_log" "$cycle" 120 0.1
    wait_for_count "accounting user=$accept_public_key status=Start" "$radius_log" "$cycle" 120 0.1
    apply_expected=$((apply_expected + 1))
    wait_for_count "executed_command type=ApplySessionPolicy peer=$accept_public_key status=Executed" "$daemon_log" "$apply_expected" 120 0.1
    ...
    sudo_pw ip netns exec "$client_ns" ping -c 1 -W 2 10.60.0.1 >/dev/null
    wait_for_condition "sudo_pw wg show '$iface_name' latest-handshakes | grep '$accept_public_key' | awk '{print \$2}' | grep -qv '^0$'" 80 0.1

    sudo_pw tc class show dev "$iface_name" >"$tmp_dir/tc-class-cycle-$cycle.txt"
    send_radius_packet coa "$radius_secret" "$accept_public_key" ingress_bps=$((55000 + cycle)) egress_bps=$((33000 + cycle))
    apply_expected=$((apply_expected + 1))
    wait_for_count "executed_command type=ApplySessionPolicy peer=$accept_public_key status=Executed" "$daemon_log" "$apply_expected" 120 0.1
    wait_for_command_output_change "$tmp_dir/tc-class-cycle-$cycle.txt" "sudo_pw tc class show dev '$iface_name'" "class htb 1:" 120 0.1

    send_radius_packet disconnect "$radius_secret" "$accept_public_key"
    wait_for_count "executed_command type=BlockPeer peer=$accept_public_key status=Executed" "$daemon_log" "$cycle" 120 0.1
    wait_for_count "executed_command type=StopAccounting peer=$accept_public_key status=Executed" "$daemon_log" "$cycle" 120 0.1
    wait_for_count "accounting user=$accept_public_key status=Stop" "$radius_log" "$cycle" 120 0.1
    ...
    if sudo_pw wg show "$iface_name" peers | grep -q "$accept_public_key"; then
        echo "peer still present after cycle $cycle disconnect" >&2
        dump_diagnostics
        exit 1
    fi
    wait_for_condition "! sudo_pw tc filter show dev '$iface_name' parent 1: | grep -q '$client_wg_ip'" 120 0.1
...
echo "phase3 real soak integration passed"
